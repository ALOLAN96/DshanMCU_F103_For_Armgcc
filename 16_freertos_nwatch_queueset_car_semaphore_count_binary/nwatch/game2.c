...
void car_game(void *argument)
{
    ...
    //g_xSemOfTicksHandle = xSemaphoreCreateCounting(3, 1);
    g_xSemOfTicksHandle = xSemaphoreCreateBinary();
    xSemaphoreGive(g_xSemOfTicksHandle);
    ...
    for (size_t i = 0; i < CAR_COUNT; i++) {
        ...
        /* 创建三个汽车任务 */
        xTaskCreate(CarTask, "Car1Task", 128, &g_cars[0], osPriorityNormal, NULL);
    xTaskCreate(CarTask, "Car2Task", 128, &g_cars[1], osPriorityNormal, NULL);
    xTaskCreate(CarTask, "Car3Task", 128, &g_cars[2], osPriorityNormal, NULL);
