...
static EventGroupHandle_t g_xCarEventGroupHandle;
static TaskHandle_t g_Car1TaskHandle, g_Car2TaskHandle, g_Car3TaskHandle;
    ...
    for (;;) {
        ...
        // if (irRecvDdata.val == pCarTaskParm->control_key)
        {
            if (pCarTaskParm->x < g_xres - CAR_LENGTH) {
                ...
                if (pCarTaskParm->x == g_xres - CAR_LENGTH) {
                    /* 发出任务通知给Car2，Car3 */
                    xTaskNotifyGive(g_Car2TaskHandle);
                    xTaskNotify(g_Car3TaskHandle, 100, eSetValueWithOverwrite);
                    vTaskDelete(NULL);
                }
            }
        }
...
void car_game(void *argument)
{
    ...
    g_xCarEventGroupHandle = xEventGroupCreate();
    ...
    xTaskCreate(Car1Task, "Car1Task", 128, &g_cars[0], osPriorityNormal, &g_Car1TaskHandle);
    xTaskCreate(Car2Task, "Car2Task", 128, &g_cars[1], osPriorityNormal1, &g_Car2TaskHandle);
    xTaskCreate(Car3Task, "Car3Task", 128, &g_cars[2], osPriorityNormal1, &g_Car3TaskHandle);
