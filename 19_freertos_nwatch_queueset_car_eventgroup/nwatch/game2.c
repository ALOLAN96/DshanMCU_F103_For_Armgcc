    ...
    for (;;) {
        ...
        // if (irRecvDdata.val == pCarTaskParm->control_key)
        {
            if (pCarTaskParm->x < g_xres - CAR_LENGTH) {
                ...
                if (pCarTaskParm->x == g_xres - CAR_LENGTH) {
                    /* 设置事件组 */
                    xEventGroupSetBits(g_xCarEventGroupHandle, (1 << 0));
                    vTaskDelete(NULL);
                }
            }
        }
...
static void Car2Task(void *parm)
{
    ...
    // /* 等待事件组通知 */
    // xEventGroupWaitBits(g_xCarEventGroupHandle, (1 << 0), pdTRUE, pdFALSE, portMAX_DELAY);
        ...
        // if (irRecvDdata.val == pCarTaskParm->control_key)
        {
            if (pCarTaskParm->x < g_xres - CAR_LENGTH) {
                ...
                if (pCarTaskParm->x == g_xres - CAR_LENGTH) {
                    /* 设置事件组 */
                    // xEventGroupSetBits(g_xCarEventGroupHandle, (1 << 0));
                    xEventGroupSetBits(g_xCarEventGroupHandle, (1 << 1));
                    vTaskDelete(NULL);
                }
            }
        }
...
static void Car3Task(void *parm)
{
    ...
    /* 等待事件组通知 */
    xEventGroupWaitBits(g_xCarEventGroupHandle, (1 << 0) | (1 << 1), pdTRUE, pdTRUE, portMAX_DELAY);
    for (;;) {
        /* 读取IR值 */
        // xQueueReceive(xQueueIRHandle, &irRecvDdata, portMAX_DELAY);
...
void car_game(void *argument)
{
    ...
    xTaskCreate(Car3Task, "Car3Task", 128, &g_cars[2], osPriorityNormal1, NULL);
