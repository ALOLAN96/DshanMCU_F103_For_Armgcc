...
static void Dispatchkey(IrRecvData *pIrRecvData)
{
#if 0
    extern QueueHandle_t g_xQueueCar1Handle, g_xQueueCar2Handle, g_xQueueCar3Handle;
    xQueueSendToBackFromISR(g_xQueueCar1Handle, &pIrRecvData, NULL);
    ...
    for (size_t i = 0; i < g_queue_cnt; i++) {
        xQueueSendToBackFromISR(g_xQueues[i], pIrRecvData, NULL);
    }
}
